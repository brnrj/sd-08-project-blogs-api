// const INVALID_ENTRIES = 'Invalid entries. Try again.';
const ALREADY_REGISTERED = 'User already registered';
// const ALL_FIELDS_REQUIRED = 'All fields must be filled';
// const INCORRECT_USER_OR_PASS = 'Incorrect username or password';
// const JWT_MALFORMED = 'jwt malformed';
// const RECIPE_NOT_FOUND = 'recipe not found';
const MISSING_TOKEN = 'Token not found';
const DISPLAY_NAME_TOO_SHORT = '"displayName" length must be at least 8 characters long';
const INVALID_EMAIL = '"email" must be a valid email';
const MISSING_EMAIL = '"email" is required';
const EMAIL_IS_EMPTY = '"email" is not allowed to be empty';
const PASSWORD_TOO_SHORT = '"password" length must be 6 characters long';
const MISSING_PASSWORD = '"password" is required';
const PASSWORD_IS_EMPTY = '"password" is not allowed to be empty';
const INVALID_FIELDS = 'Invalid fields';
const INVALID_TOKEN = 'Expired or invalid token';
const USER_NOT_FOUND = 'User does not exist';
const MISSING_NAME = '"name" is required';
const MISSING_TITLE = '"title" is required';
const MISSING_CONTENT = '"content" is required';
const MISSING_CATEGORY_ID = '"categoryIds" is required';
const CATEGORY_IDS_NOT_FOUND = '"categoryIds" not found';

module.exports = {
  // INVALID_ENTRIES,
  // ALREADY_REGISTERED,
  // ALL_FIELDS_REQUIRED,
  // INCORRECT_USER_OR_PASS,
  // JWT_MALFORMED,
  // RECIPE_NOT_FOUND,
  MISSING_TOKEN,
  DISPLAY_NAME_TOO_SHORT,
  INVALID_EMAIL,
  MISSING_EMAIL,
  EMAIL_IS_EMPTY,
  PASSWORD_TOO_SHORT,
  MISSING_PASSWORD,
  PASSWORD_IS_EMPTY,
  ALREADY_REGISTERED,
  INVALID_FIELDS,
  INVALID_TOKEN,
  USER_NOT_FOUND,
  MISSING_NAME,
  MISSING_TITLE,
  MISSING_CONTENT,
  MISSING_CATEGORY_ID,
  CATEGORY_IDS_NOT_FOUND,
};
